...
void list_stops_line(Line lines[], char name_line[]){
    ...
    for(i = 0; lines[i].name_line[0] != '\0'; i++){
        if(strcmp(lines[i].name_line, name_line) == 0){
            if(lines[i].number_stops == 0) return;
            for(j = 0; j < lines[i].number_stops - 1; j++)
...
void list_inverted_stops_line(Line lines[], char name_line[]){
    ...
    for(i = 0; lines[i].name_line[0] != '\0'; i++){
        if(strcmp(lines[i].name_line, name_line) == 0){
            if(lines[i].number_stops == 0) return;
            printf("%s, ", lines[i].last_stop);
            for(j = (lines[i].number_stops - 2); j > 0; j--) 
                printf("%s, ", lines[i].stops[j]);
            printf("%s\n", lines[i].stops[0]);
            break;
        }
...
void list_lines(Line lines[]){
    ...
    for(i = 0; lines[i].name_line[0] != '\0'; i++){
        if(lines[i].first_stop[0] == '\0')
            printf("%s %d %.2f %.2f\n", lines[i].name_line, lines[i].number_stops, lines[i].cost, lines[i].duration);
        else printf("%s %s %s %d %.2f %.2f\n", lines[i].name_line, lines[i].first_stop, lines[i].last_stop, lines[i].number_stops, 
...
Receives an array of structs stops.*/
void list_stops(Stop stops[]){
    int i;
    for(i = 0; stops[i].name_stop[0] != '\0'; i ++)
        printf("%s: %16.12f %16.12f %d\n", stops[i].name_stop, stops[i].latitude, stops[i].longitude, stops[i].number_lines);
}
...
/*Updates the stops, the cost, the duration and the number of stops of the corresponding line in the array of structs lines when the link is added to the end
of the line and when the line doesn't have any stops.
    ...
    for(i = 0; i < MAXLINE; i++){
        if(strcmp(lines[i].name_line, name_line) == 0){
            if(lines[i].first_stop[0] == '\0'){
                ...
                lines[i].number_stops = 1;
            }
            strcpy(lines[i].stops[lines[i].number_stops - 1], first_stop);
            strcpy(lines[i].last_stop, last_stop);
            lines[i].cost += cost;
            lines[i].duration += duration;
            lines[i].number_stops++;
        }
    }
}

/*Updates the stops, the cost, the duration and the number of stops of the corresponding line in the array of structs lines when the link is added to the 
...
void update_inv_line(Line lines[], char name_line[], char first_stop[], float cost, float duration){
    ...
    for(i = 0; lines[i].name_line[0] != '\0'; i++){
        if(strcmp(lines[i].name_line, name_line) == 0){
            for(j = lines[i].number_stops; j > 0; j--)
...
/*Updates the array of lines and the number of lines that pass in the corresponding stop.
Receives an array of structs stops and two strings, the name of the line and the name of the stop.*/
void update_stop(Stop stops[], char name_line[], char stop[]){
    int i, j = 0, n = 1;
    for(i = 0; stops[i].name_stop[0] != '\0'; i++){
        if(strcmp(stops[i].name_stop, stop) == 0){
            if(stops[i].lines[j][0] == '\0'){
                strcpy(stops[i].lines[j], name_line);
                stops[i].number_lines = 1;
            }
            else{
                for(j = 0; stops[i].lines[j][0] != '\0'; j++){
                    if(strcmp(stops[i].lines[j], name_line) == 0){
                        n = 0;
                        break;
                    } 
                }
...
Receives as arguments a string of input from the user and an array of structs stops.*/
...
void command_c(char command[], Line lines[]){
    ...
    else if(strcmp(inverse, "inverso") == 0 || strcmp(inverse, "invers") == 0 || strcmp(inverse, "inver") == 0 || strcmp(inverse, "inve") == 0
        || strcmp(inverse, "inv") == 0)
...
void command_l(char command[], Line lines[], Stop stops[]){
    ...
    else if(checks_no_stops_line(lines, name_line) == 1 || its_last_stop(lines, name_line, first_stop) == 1){
...
void bubble(char name_of_lines[MAXLINE][MAXNAMEL], int num) {
    int i, j, change = 1;
    for (i = 0; change && i < num - 1; i++) {
        change = 0;
	for (j = 0; j < num - 1 - i; j++) {
	    if (strcmp(name_of_lines[j], name_of_lines[j+1]) > 0) {
	        char aux[MAXNAMEL];
            strcpy(aux, name_of_lines[j]);
	        strcpy(name_of_lines[j], name_of_lines[j+1]);
		strcpy(name_of_lines[j+1], aux);
		change = 1;
	    }
	}
    }
...
void list_intersections(Stop stops[]){
    int i, m;
    for(i = 0; stops[i].name_stop[0] != '\0'; i++){
        if(stops[i].number_lines > 1){
            bubble(stops[i].lines, stops[i].number_lines);
            printf("%s %d: ", stops[i].name_stop, stops[i].number_lines);
            for(m = 0; m < stops[i].number_lines; m++){
                if(m == stops[i].number_lines - 1)
                    printf("%s\n", stops[i].lines[m]);
                else printf("%s ", stops[i].lines[m]);
            }
        }       
    }
}
